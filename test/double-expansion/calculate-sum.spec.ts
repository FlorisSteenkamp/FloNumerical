...
import { calculateSum } from '../../src/double-expansion/calculate-sum';
import { split } from '../../src/double-expansion/split';


function check(sum: number[][], expected: number) {
    let result = calculateSum(sum);
    expect(estimate(result)).to.eql(expected);
}
